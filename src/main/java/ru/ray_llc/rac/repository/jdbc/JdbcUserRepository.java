package ru.ray_llc.rac.repository.jdbc;
...
public class JdbcUserRepository implements UserRepository {
  ...
  private static final BeanPropertyRowMapper<User> ROW_MAPPER = BeanPropertyRowMapper
  ...
  public User save(User user) {
    ...
    if (user.isNew()) {
      ...
      if (namedParameterJdbcTemplate.update("""
             ...
             registered=:registered, enabled=:enabled, phone=:phone WHERE id=:id
          """, parameterSource) == 0) {
        return null;
      }
...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
  ...
  public List<User> getAll() {
    List<User> users = jdbcTemplate.query("SELECT * FROM users ORDER BY name, login", ROW_MAPPER);
  ...
  private void insertRoles(User u) {
    ...
    if (!CollectionUtils.isEmpty(roles)) {
      jdbcTemplate
          .batchUpdate("INSERT INTO user_roles (user_id, role) VALUES (?, ?)", roles, roles.size(),
              (ps, role) -> {
                ps.setInt(1, u.id());
  ...
  private User setRoles(User u) {
    if (u != null) {
      List<Role> roles = jdbcTemplate
          .queryForList("SELECT role FROM user_roles  WHERE user_id=?", Role.class, u.getId());
      u.setRoles(roles);
    }
    return u;
  }
